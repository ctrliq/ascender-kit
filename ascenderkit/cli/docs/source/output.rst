...
By default, ascender prints valid JSON for successful commands.  The ``-f`` (or
...
.. code:: bash
    ...
    ascender jobs list -f human
    ascender jobs list -f human --filter name,created,status
    ascender jobs list -f human --filter *
