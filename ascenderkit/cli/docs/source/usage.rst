...
.. code:: bash
    ...
    ascender [<global-options>] <resource> <action> [<arguments>]
    ascender --help

The ``resource`` is a type of object within Ascender (a noun), such as ``users`` or ``organizations``.

The ``action`` is the thing you want to do (a verb). Resources generally have a base set of actions (``get``, ``list``, ``create``, ``modify``, and ``delete``), and have options corresponding to fields on the object in Ascender.  Some resources have special actions, like ``job_templates launch``.


Getting Started
---------------

Using |prog| requires some initial configuration.  Here is a simple example for interacting with an Ascender server:

.. code:: bash

    ascender --conf.host https://ascender.example.org \
        --conf.username joe --conf.password secret \
        --conf.insecure \
        users list

There are multiple ways to configure and authenticate with an Ascender server.  For more details, see :ref:`authentication`.
...
.. code:: bash
    ...
    ascender --conf.host https://ascender.example.org --help

To get a description of a specific resource, and list its available actions (and their arguments):

.. code:: bash

    ascender --conf.host https://ascender.example.org users --help
    ascender --conf.host https://ascender.example.org users create --help
