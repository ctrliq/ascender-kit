...
from ascenderkit.cli.format import FORMATTERS, format_response, add_authentication_arguments, add_formatting_import_export
from ascenderkit.cli.utils import CustomRegistryMeta, cprint
...
class Import(CustomCommand):
    name = 'import'
    help_text = 'import resources into Ascender'

    def handle(self, client, parser):
        if parser:
            parser.usage = 'ascender import < exportfile'
...
class Export(CustomCommand):
    name = 'export'
    help_text = 'export resources from Ascender'
    ...
    def handle(self, client, parser):
        ...
        parser.usage = 'ascender export > exportfile'
