...
from ascenderkit.exceptions import NoContent
...
class SettingsModify(CustomAction):
    ...
    def add_arguments(self, parser, resource_options_parser):
        ...
        # A user who may not change settings is served OPTIONS with no PUT
        # section, and so has no keys to be offered. Reaching straight into it
        # raised KeyError('PUT') here, which took the whole resource down with
        # it: `settings list` reads nothing and still could not run.
        settable = options.json.get('actions', {}).get('PUT', {})
        parser.choices['modify'].add_argument('key', choices=sorted(settable.keys()), metavar='key', help='')
