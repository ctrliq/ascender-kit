...
from ascenderkit.words import words
from ascenderkit.exceptions import WaitUntilTimeout
...
def seconds_since_date_string(date_str, fmt='%Y-%m-%dT%H:%M:%S.%fZ', default_tz=UTC()):
    ...
    :param fmt: Formatting string - by default, this value is set to parse
        date strings originating from ascender API response data.
...
def args_string_to_list(args):
    ...
    # shlex yields str, so there is nothing here to decode. Calling .decode on
    # the tokens dates from Python 2 and raised AttributeError on every
    # non-empty input; only the empty string got through, by never looping.
    return list(lexer)
