        ...
        for resource in EXPORTABLE_RESOURCES:
            ...
            if all_resources or value is not None:
                ...
                if value:
                    ...
                    if not endpoint.results:
                        # Naming something that is not there is almost always a
                        # typo, and the export it produces is an empty list that
                        # looks just like a successful one.
                        log.warning("No %s matched %r, so none were exported.", resource, value)
        ...
        for asset in assets:
            ...
            for field, value in asset.items():
                ...
                if post_fields[field]['type'] in ('id', 'integer') and isinstance(value, dict):
                    ...
                    # Keep any $encrypted$ placeholder for now. Whether it can be
                    # honoured depends on the object already existing, which is
                    # only known once the cache has been consulted below.
                    post_data[field] = value

            _page = self._cache.get_by_natural_key(asset['natural_key'])
            try:
                if _page is None:
                    # Creating: there is no stored secret to preserve, so the
                    # placeholders are dropped rather than sent as empty strings.
                    post_data = utils.drop_encrypted(post_data)
                    ...
                    if asset['natural_key']['type'] == 'user':
                        # PUT would drop any field the export did not carry,
                        # including the password. See ansible/awx#14053.
        ...
        for _page, name, related_set in self._related:
            ...
            if 'natural_key' not in related_set[0]:  # It is an attach set
                ...
                for item in related_set:
                    ...
                    if rel_page is None:
                        log.error("Could not find matching object in Ascender for imported relation, item: %r", item)
                        self._has_error = True
                        continue
