...
from ascenderkit.utils import random_title, update_payload
from ascenderkit.api.mixins import HasStatus
from . import base
from . import page
...
class UnifiedJobTemplate(HasStatus, base.Base):
    ...
    def add_schedule(self, name='', description='', enabled=True, rrule=None, **kwargs):
        ...
        payload = dict(name=name or f"{self.name} Schedule {random_title()}", description=description or random_title(10), enabled=enabled, rrule=str(rrule))
...
page.register_page([resources.unified_job_templates, resources.execution_environment_unified_job_templates], UnifiedJobTemplates)
