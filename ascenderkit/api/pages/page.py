...
from ascenderkit.utils import PseudoNamespace, is_relative_endpoint, are_same_endpoint, super_dir_set, is_list_or_tuple, to_str
...
def register_page(urls, page_cls):
    if not _page_registry.default:
        from ascenderkit.api.pages import Base
...
class Page(object):
    ...
    # other than JSON set this rather than overriding get(), which is easy to
    # do without carrying the all_pages handling across.
    get_headers = None
    ...
    def get(self, all_pages=False, **query_parameters):
        r = self.connection.get(self.endpoint, query_parameters, headers=self.get_headers)
        page = self.page_identity(r)
        if all_pages and getattr(page, 'next', None):
            paged_results = [r.json()['results']]
            while page.next:
                r = self.connection.get(self.next, headers=self.get_headers)
                page = self.page_identity(r)
