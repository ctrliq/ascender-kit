...
def test_seconds_since_date_string(date_string, now, expected):
    with mock.patch('ascenderkit.utils.utcnow', return_value=now):
...
class TestPollUntil(object):
    ...
    def test_exc_raised_on_timeout(self):
        with mock.patch('ascenderkit.utils.logged_sleep'):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('a b c', ['a', 'b', 'c']),
        ('a  b', ['a', 'b']),
        ('"quoted arg" b', ['quoted arg', 'b']),
        ("a 'b c'", ['a', 'b c']),
        ('sh -c "echo hello world"', ['sh', '-c', 'echo hello world']),
        ('ansible-playbook -i inv site.yml -e "k=v"', ['ansible-playbook', '-i', 'inv', 'site.yml', '-e', 'k=v']),
    ],
)
def test_args_string_to_list(line, expected):
    """Every non-empty input used to raise AttributeError.

    The tokens were being .decode()d, which is a Python 2 habit; shlex yields
    str. Only '' escaped it, by never entering the loop.
    """
    assert utils.args_string_to_list(line) == expected


@pytest.mark.parametrize('tokens', [['a', 'b c'], ['ansible-playbook', '-e', 'k=v'], ['x"y']])
def test_args_string_to_list_reverses_list2cmdline(tokens):
    # The docstring claims to be the reverse of subprocess.list2cmdline, and it
    # is, short of a token holding an apostrophe: list2cmdline leaves that
    # unescaped and posix shlex then reads it as an unterminated quote.
    import subprocess

    assert utils.args_string_to_list(subprocess.list2cmdline(tokens)) == tokens


def test_args_string_to_list_rejects_an_unterminated_quote():
    with pytest.raises(ValueError):
        utils.args_string_to_list("it's")
